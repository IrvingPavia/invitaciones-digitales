        ...
        @if (config.cards && config.cards.length > 0) {
          <div class="dresscode-examples">
            @for (card of config.cards; track card.id) {
              <div class="example-card reveal" [class.no-bg]="config.showCardBg === false" [style.border-radius.px]="card.cardBorderRadius ?? 16" [style.--card-bg-opacity]="(config.cardBgOpacity ?? 100) / 100" [style.border-style]="getCardBorderStyle()" [style.border-width.px]="getCardBorderWidth()" [style.box-shadow]="getCardBoxShadow()" [style.clip-path]="getCardClipPath()" [class.neon-border]="getIsNeon()">
