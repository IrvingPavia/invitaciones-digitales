  ...
  template: `
    <div>
        ...
        @if (events().length > 1) {
          <div class="card mb-16">
            <div class="flex gap-12" style="align-items:center;flex-wrap:wrap">
              ...
              <select (change)="selectEvent($event)" style="background:rgba(255,255,255,0.05);border:1px solid rgba(124,92,191,0.3);border-radius:8px;padding:8px 12px;color:white;font-size:14px;flex:1;max-width:300px">
        ...
        @if (selectedEvent()) {
          <div class="client-actions mb-24">
            @if (selectedEvent()!.event_mode === 'open') {
              <a [routerLink]="['/dashboard/registrations', selectedEvent()!.id]" class="client-action-card">
        ...
        @if (events().length > 0) {
          <div class="card mb-16">
            <div class="flex gap-12" style="align-items:center;flex-wrap:wrap">
              ...
              @if (selectedEvent()) {
                @if (selectedEvent()!.event_mode === 'open') {
                  <a [routerLink]="['/dashboard/registrations', selectedEvent()!.id]" class="btn btn-secondary btn-sm">
      ...
      @if (kpis()) {
        @if (isOpenEvent()) {
          <!-- Open event KPIs -->
          <div class="grid-4 mb-24" style="grid-template-columns:repeat(auto-fit, minmax(180px, 1fr))">
            <div class="kpi-card">
            ...
            @if (registrationStats().capacity) {
              <div class="kpi-card">
                ...
                <div><div class="kpi-value" style="color:#5cb85c">{{ registrationStats().capacity! - registrationStats().registered }}</div><div class="kpi-label">Lugares disponibles</div></div>
              </div>
              <div class="kpi-card">
          ...
          <!-- Progress bar for open events -->
          @if (registrationStats().capacity) {
            <div class="card">
              <div class="flex-between mb-16">
                <span style="font-size:14px;color:rgba(255,255,255,0.7)">Progreso de registro</span>
              ...
              <div style="background:rgba(255,255,255,0.1);border-radius:8px;height:12px;overflow:hidden">
                <div [style.width.%]="registrationPct()" style="height:100%;background:linear-gradient(90deg,var(--gold),var(--gold-light));border-radius:8px;transition:width 0.5s ease"></div>
              </div>
          ...
          <!-- Progress bar for private events -->
